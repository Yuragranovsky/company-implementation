...
void setCompany() {
	...
	@Test
	void saveRestoreTest() throws Exception {
		company.save();
		Company companyRestored = new Company(new Employee[0]);
		companyRestored.restore();
		Employee[] expected = {empl2, empl1, empl3};
		Iterator<Employee> it = companyRestored.iterator();
		int index = 0;
		while(it.hasNext()) {
			assertEquals(expected[index++], it.next());
		}
